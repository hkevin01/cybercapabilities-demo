...
db.exec('CREATE TABLE IF NOT EXISTS comments (id INTEGER PRIMARY KEY, content TEXT)');
const userCount = db.prepare('SELECT COUNT(*) as c FROM users').get().c;
if (userCount === 0) {
  db.prepare('INSERT INTO users(username, password) VALUES (?, ?)').run('admin', 'admin123'); // demo only
}
...
app.post('/login', async (req, res) => {
  ...
  const row = db.prepare('SELECT username FROM users WHERE username = ? AND password = ?').get(username, password);
...
app.post('/comment', (req, res) => {
  ...
  const parsed = schema.safeParse(req.body || {});
  if (!parsed.success) return res.status(400).send('Invalid input');
  db.prepare('INSERT INTO comments(content) VALUES (?)').run(parsed.data.content);
  res.redirect('/comments');
});
app.get('/comments', (req, res) => {
  const rows = db.prepare('SELECT id, content FROM comments ORDER BY id DESC').all();
